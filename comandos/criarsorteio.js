...
exports.run = async (client, message, args) => {
    ...
    if(args[0]===undefined)
    {
       ...
       await coletarDados(novaSala, salaOrigem);
...
async function printar(sala, objeto) {
    ...
    let msg = await sala.send(embed);
    await msg.react(`🎉`);
    return;
}


async function coletarDados(sala, salaOrigem) {
