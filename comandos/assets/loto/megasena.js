...
module.exports = async (args) => {
  ...
  await request(url, { jar: true })
    .then(await stopSpinner)
    .then(await parseResponseToJson)
    .then(await verifyError)
    // .then(await extractLotteryValuesFromBody)
    // .then(await responseConsole)
    .catch(errorHandler)
  // console.log("Corpo", corpo)
  return corpo
}

const stopSpinner = async (body) => {
  await spinner.stop()
  return body
}

const parseResponseToJson = async (response) => {
  corpo = await JSON.parse(response)
  return corpo
}

const verifyError = async body => {
...
// const extractLotteryValuesFromBody = async(body) => {
...
// const responseConsole = async (body) => {
